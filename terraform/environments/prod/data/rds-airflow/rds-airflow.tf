...
terraform {
  ...
  backend "s3" {
    ...
    key    = "data/rds-airflow/rds-airflow"
    region = "us-east-1"
  }
}
...
data "aws_ssm_parameter" "username" {
  name = "/prod/rds-airflow/USERNAME"
}

data "aws_ssm_parameter" "password" {
  name = "/prod/rds-airflow/PASSWORD"
}

# TODO:  Lock down to airflow service security group; protected from public access for now though
...
module "rds" {
  ...
  monitoring_role_name                = "${var.environment}-${var.project}-${var.application}-monitoring"
